...
sub Data {
    ...
    $Self->{Translation}->{'If a note is added by an agent, sets the state of a ticket in the additional ITSM field screen of the agent interface.'} =
        'Setzt den Status eines Tickets in der Ansicht "Zusätzliche ITSM-Felder" im Agenten-Interface, wenn eine Notiz von einem Agenten hinzugefügt wird.';
    $Self->{Translation}->{'If a note is added by an agent, sets the state of a ticket in the decision screen of the agent interface.'} =
        'Setzt den Status eines Tickets in der Ansicht „Entscheidung“ im Agenten-Interface, wenn ein Agent eine Notiz hinzufügt.';
    ...
    $Self->{Translation}->{'Shows a link in the menu to change the decision of a ticket in its zoom view of the agent interface.'} =
        'Zeigt in der Agentenoberfläche im Ticket-Menü einen Link an, um die Entscheidung an einem Ticket zu ändern.';
